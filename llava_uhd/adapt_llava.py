...
class adapt_LlavaMetaModel:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.config.resampler_seq_len = getattr(model_args, 'resampler_seq_len', 64)
        self.config.resampler_embed_dim = getattr(model_args, 'resampler_embed_dim', 2048)
        self.config.resampler_num_heads = getattr(model_args, 'resampler_num_heads', 8)
        self.config.resampler_kv_dim = getattr(model_args, 'resampler_kv_dim', 1024)
        self.config.mm_unfreeze_position_embedding = getattr(model_args, 'mm_unfreeze_position_embedding', False)
...
class adapt_LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images,origin_image_widths,origin_image_heights):
        ...
        if not isinstance(images, list):
            num_slices = images.shape[1] // 3
            for i, image in enumerate(images.chunk(num_slices, dim=1)):
                if i != num_slices - 1:
                    image_features.append(vision_tower(image, slice_w_nums, slice_h_nums).unsqueeze(1))
                else:
                    image_features.append(vision_tower(image, abstract_w_nums, abstract_h_nums).unsqueeze(1))
                image_features_new = []
                num_batchs = len(image_features[0])
                for b in range(num_batchs):
                    for s in range(num_slices):
                        image_features_new.append(image_features[s][b])
                image_features = torch.concat(image_features_new, dim=0)
        else:
            for i in range(len(images)):
                num_slices = images[i].shape[0] // 3
                for j, image in enumerate(images[i].chunk(num_slices, dim=0)):
                    if j != num_slices - 1:
                        image_features.append(vision_tower(image.unsqueeze(0), [slice_w_nums[i]],[slice_h_nums[i]]))
                    else:
                        image_features.append(vision_tower(image.unsqueeze(0), [abstract_w_nums[i]],[abstract_w_nums[i]]))
            image_features = torch.concat(image_features, dim=0)
        
        image_features = self.get_model().mm_projector(image_features, origin_image_widths, origin_image_heights)
...
AutoConfig.register("adapt_llava", LlavaConfig)
